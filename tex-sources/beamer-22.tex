...
\begin{frame}{Communicating classes}
\begin{Definition}
...
\begin{itemize}
\item [Reflexivity:]<2-> Since $P^0 = I$, it follows that $p_{xx}^{(0)} = 1 > 0$ for each state $x\in\sX$, and hence $x\leftrightarrow x$. % and the reflexivity of the relation follows. 
\item [Symmetry:]<3-> If $x \leftrightarrow y$, then we know that $x \to y$ and $y \to x$ and hence $y \leftrightarrow x$. 
\item [Transitivity:]<4-> For transitivity, suppose $x \leftrightarrow y$ and $y \leftrightarrow z$. 
...
\item<4-> An \textbf{open} communicating class is not closed, i.e. there exist an edge that leaves this class. 
...
\begin{frame}{Irreducibility and periodicity}
\begin{Definition} 
...
\begin{align*}
1-f_{yy} = P_y\set{\tau_X^{\set{y},1} = \infty} 
&\ge P_y\set{\tau_X^{\set{y},1} = \infty, \tau_X^{\set{x},1}  = n} \\
&= P_x\set{\tau_X^{\set{y},1} = \infty}P_y\set{\tau_X^{\set{x},1}  = n} = f_{yx}^{(n)}(1-f_{xy}). % > 0.
\end{align*}
