...
Therefore, 
\EQ{
\E[XY] = \int_{(x,y) \in \R^2}xydF_{X,Y}(x,y) = \int_{x \in \R}xdF_X(x)\int_{y \in \R}ydF_Y(y) = \E[X]\E[Y].
}
\end{proof}
\begin{shaded}
\begin{exmp}[Uncorrelated dependent random variables] 
Let $X: \Omega \to \R$ be a continuous random variable with even density function $f_X:\R \to \R_+$, 
...
To show dependence of $X$ and $Y$, 
we take positive $x,y$ such that $F_X(x) < 1$ and $x > x_y$ where $\set{x_y} = g^{-1}(y)\cap \R_+$. 
Then, we can write the set 
\EQ{
A_Y(y) = Y^{-1}(-\infty, y] 
%= \set{\omega\in\Omega: Y(\omega) \le y} 
%= \set{\omega \in \Omega: g(X(\omega)) \le y} 
%= \set{\omega \in \Omega: -x_y \le X(\omega) \le x_y}
= X^{-1}[-x_y, x_y].
...
Since $X$ has even density function, we have $f_X(x) = f_X(-x)$ for all $x \in \R$. 
Therefore, we have
\EQ{
\E Xg(X)\SetIn{X < 0} 
...
The last equality follows from the fact that $g$ and $f_X$ are even. 
%Further, since the function $g$ is even, we have $g(X) = g(-X)$. 
Therefore, we have
\EQ{
\E[Xg(X)] 
...
\end{defn}
\begin{thm}[Correlation coefficient] 
...
\end{thm}
\begin{proof} 
For two random variables $X$ and $Y$, we can define normalized random variables $W \triangleq \frac{X-\E X}{\sqrt{\Var(X)}}$ and $Z \triangleq \frac{Y-\E Y}{\sqrt{\Var(Y)}}$. 
Applying the AM-GM inequality to random variables $W, Z$, we get 
\EQ{
\abs{\cov(X,Y)}  \le \sqrt{\Var(X) \Var(Y)}. 
}
Recall that equality is achieved iff $W = Z$ almost surely or equivalently iff $X = \alpha Y + \beta$ almost surely.  
...
%\begin{defn}
%We define a function $\norm{}_p: L^p \to \R_+$ defined by $\norm{}_p(X) = \norm{X}_p \triangleq (\E\abs{X}^p)^{1/p}$ for any $X \in L^p$ and real $p \ge 1$. 
%\end{defn}
%\begin{defn} 
%Given a vector space $V$ of random variables, a \textbf{norm} on the vector space is a map $f: V \to \R_+$ 
%such that 
%\begin{enumerate}
%	\item [\textbf{homogeneity:}] $f(aX) = \abs{a}f(X)$ for all $a \in \R$ and $X \in V$,
%	\item [\textbf{sub-additivity:}] $f(X+Y) \le f(X) + f(Y)$ for all $X,Y \in V$, and 
%	\item [\textbf{point-separating:}] $f(X) \ge 0$ for all $X \in V$. 
%\end{enumerate}
%\end{defn}
%\begin{shaded}
%\begin{exmp}
...
%and hence $L^\infty$ is the vector space of bounded random variables. 
%It follows that the function $\norm{}_p$ is a norm for vector space $L^p$ for $p \in \set{1,\infty}$. 
%We will also show that $\norm{}_p$ is a norm for all $p \ge 1$. 
%\end{exmp}
%\end{shaded}
...
%Consider two random variables $X, Y$ such that $\E\abs{X}^p$ and $\E\abs{Y}^q$ are finite for $p, q \ge 1$ such that $\frac{1}{p} + \frac{1}{q} = 1$. 
Then, 
\EQ{
...
\end{thm}
\begin{proof}
Consider a random variable $Z: \Omega \to \set{p\ln v, q \ln w}$ with probability mass function $\set{\frac{1}{p}, \frac{1}{q}}$. 
...
It follows that for any random variables $V,W$, we have $VW \le \frac{V^p}{p} + \frac{W^q}{q}$. 
...
\end{thm}
\begin{proof}
Since addition is a Borel measurable function, $X+Y$ is a random variable. 
...
\le \frac{1}{2}g(\abs{X}) + \frac{1}{2}g(\abs{Y})
= \frac{1}{2}\abs{X}^p + \frac{1}{2}\abs{Y}^p.
}
...
\EQ{
\norm{X+Y}_p^p 
= \E[\abs{X+Y}\abs{X+Y}^{p-1}]
\le \E([\abs{X}+\abs{Y})\abs{X+Y}^{p-1}]
= \E\abs{X}\abs{X+Y}^{p-1} +  \E\abs{Y}\abs{X+Y}^{p-1}.
}
From the H\"older's inequality applied to conjugate pair $(p,q)$ to the two products on RHS, we get 
\EQ{
\norm{X+Y}_p^p \le (\norm{X}_p+\norm{Y}_p)\norm{\abs{X+Y}^{p-1}}_q
}
Recall that $q = \frac{p}{p-1}$. %, and hence $(p-1)q = p$. 
