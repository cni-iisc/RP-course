...
{{< video https://www.youtube.com/watch?v=OpsLU8dFIY8&list=PLNN9TCnjABcY562vG5XuSfodGs6_BMcAF >}}
...
:::: shaded
::: exmp
**Example 2** (Conditional distribution). []{#exmp:FairDieConditionHalf label="exmp:FairDieConditionHalf"} Consider the probability space $(\Omega, \sF, P)$ corresponding to a random experiment where a fair die is rolled once. For this case, the outcome space $\Omega = [6]$, the event space $\sF = \cP([6])$, and the probability measure $P(\omega) = \frac{1}{6}$ for all $\omega\in \Omega$.
...
:::: shaded
::: exmp
**Example 4** (Conditional expectation). For the random variable $X$ and event $B$ defined in Example [\[exmp:FairDieConditionHalf\]](#exmp:FairDieConditionHalf){reference-type="ref" reference="exmp:FairDieConditionHalf"}, the conditional expectation $\E[X|B] = 2$.
:::
::::
